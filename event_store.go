...
func (es *eventStore) SummarizeBatchEvents() {
	...
	for _, evt := range evtsToAdd {
		evtBase := EventBase{
			0,
			serviceNameMap[evt.Service].Id,
			evt.Type,
			evt.Name,
			0,
			envNameMap[evt.Environment].Id,
			evt.Data,
			util.Hash(evt.Data),
		}
		_, err := es.ds.AddEventBaseRework(evtBase)
		if err != nil {
			fmt.Println(err)
		}
...
func (es *eventStore) OpsdbQuery(from string, to string, envId string, serviceId string, groupId string) ([]OpsdbResult, error) {
	now := time.Now()
	defer func() {
		metrics.EventStoreLatency("CountEvents", now)
	}()
