...
CREATE TABLE IF NOT EXISTS event_base (
  ...
  PRIMARY KEY (service_id, event_type, processed_data_hash)
);
...
CREATE TABLE IF NOT EXISTS event_detail (
  ...
  PRIMARY KEY (processed_detail_hash)
);
...
--   event_data_id int8 REFERENCES event_data(_id),
  ...
  counter_json json,
  PRIMARY KEY (event_instance_id, start_time, end_time)
);
